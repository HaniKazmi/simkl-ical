 ...
 *
 * Owns everything the feed is built from and produces — calendars, film
 * release dates, events, the rendered ICS. The library is not here: both
 * halves share it, so `Orchestrator` owns it and passes it in. One owner per
 * piece of state keeps a poll from having two copies that disagree.
 */
...
export class Feed {
   ...
   * recently" from "never asked". A retryably-failed lookup keeps its old
   * stamp, so the film stays due and the next poll retries exactly the films
   * that failed, with no flag to carry.
   */
  filmStamps = new Map<number, Temporal.Instant>();
  // calendarsAt advances on every attempt, cache-served ones included; only
  // calendarsFreshAt means the CDN answered.
  calendarsAt: string | null = null;
  calendarsFreshAt: string | null = null;
  /**
   * When the CDN last sent new bytes, as opposed to a 304. Only this says a
   * refresh did any work. Not in `/healthz`: a run of 304s is the healthy
   * outcome, not a condition, and the payload's key order is pinned.
   */
  ...
  servingCached = false;
  /**
   * The two failures this half can have. Owned here rather than in a flat bag
   * shared with the library and the sheet, so timers cannot clear each other's
   * failures by construction rather than by rule.
   */
  ...
  /**
   * Serve the last saved feed until a complete fresh one renders, and warm
   * the calendars. Nothing else is restored, so no control state outlives the
   * process. Nothing renders here: at boot there is no library yet and the
   * render gate would decline.
   */
  async hydrate({ signal }: { signal: AbortSignal }): Promise<void> {
    // Contained here. `loadFeed` throws on an unreadable file so it is
    // visible, but letting that escape would skip the calendar fetch and the
    // first poll after it — a full timer interval lost over a fallback file.
  ...
  /**
   * FETCH, and nothing else — rendering is the caller's.
   *
   * This fetch is several MB and takes seconds to minutes, with the library
   * poll running throughout. A render queued here would carry a library read
   * from *before* the fetch, then land after the poll's own render and
   * overwrite it. The caller reads the library after this returns, so it
   * renders what is current.
   */
  ...
  /**
   * Whether any film needs a lookup, from memory alone. Public and asked by
   * `Orchestrator`: the quiet-poll early return needs the answer *before* it
   * knows whether it will resolve anything.
   */
  ...
  /**
   * FETCH, per film. Returns whether the round was complete — an incomplete
   * one must arm the caller's retry.
   *
   * Deliberately not wrapped in try/catch: these are SIMKL calls, so an
   * account-level failure belongs in `errors.library` and must reach the
   * caller. Swallowing it would make a revoked token during film lookups
  ...
  async resolveFilms(library: Library | null, { signal }: { signal: AbortSignal }): Promise<boolean> {
    ...
    // Stamped only where retrying cannot help: an unrefreshed stamp keeps the
    // film due, which is the whole retry mechanism. `unavailable` is stamped
    // because it is settled — the id fails identically forever, and unstamped
    // it would be asked about on every poll for good.
    const retryable = new Set(lookups.failed);
    for (const id of due) if (!retryable.has(id)) this.filmStamps.set(id, now);
    // Films off the list drop their stamps, or re-adding one would find it
    // fresh and never look it up. Still-planned is the whole test: a planned
    // film with no announced date is absent from `releases` and must keep its
    // stamp, or the refresh floor would never hold for it.
    const planned = new Set(filmIds);
    for (const id of this.filmStamps.keys()) if (!planned.has(id)) this.filmStamps.delete(id);

    // The threaded instant, not a fresh read: the round is stamped with when
    // it started, which is what `filmDue` measures its floor from.
  ...
  /**
   * JOIN → RENDER → SAVE, with failures contained in `errors.render`.
   *
   * Serialised: both refresh timers end here, and at default intervals the
   * library polls twelve times per calendar refresh, so overlapping runs are
   * the norm — unserialised they race on the disk save. The library is
   * captured at call time, not when the queued render runs, so a render
