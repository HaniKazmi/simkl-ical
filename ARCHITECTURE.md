...
airing, because a filled cell is one the close can never correct: the blank is what lets the season's
own average reach it later, and `Length` reading zero until then is the price of that. Once the season
has finished airing its lengths are settled, so a row added even one episode in carries the average
at once — the runtime follows the airing, the `End` date follows the watching. The runtime additionally needs `TVDB_API_KEY`; without it the other three behave
...
| A season's episode lengths | that season is completing with a blank runtime cell, or has finished airing on the run that adds its row — watched out or one episode in, since lengths settle with the airing — then never again | `GET api4.thetvdb.com/v4/series/{id}/episodes/official?season={n}` — one call is one whole season |
